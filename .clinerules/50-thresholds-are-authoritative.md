...
|---|---|---|
| `patch-health.sh` pass count | `Pass: 13` | `scripts/patch-health.sh --source release` |
| `patch-health.sh` fail count | `Fail: 0` | same |
| SDK conflicts | `0 SDK conflicts` | same |
| SDK files installed | `264 files to install` | `scripts/apply-to-tree.sh` |
| Patch buckets | `vyos/` (3) → `ask/` (8) → `fixes/` (4+) | `release/patches/` |

## Hard rules

1. **Never weaken an assertion to make a failing build pass.** If a check fails, fix the cause; do not edit the threshold.
2. The `264 files` count changes only when SDK sources are deliberately re-imported. Such a change requires:
...
3. The `Pass: 13` count changes only when a new persistent patch is added or one is removed. Each change is its own commit.
