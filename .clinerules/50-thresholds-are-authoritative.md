...
|---|---|---|
| `patch-health.sh` pass count | `Pass: 15` | `scripts/patch-health.sh --source release` |
...
3. The `Pass: 15` count changes only when a new persistent patch is added or one is removed. Each change is its own commit.
