...
|---|---|---|
| `patch-health.sh` pass count | `Pass: 16` | `scripts/patch-health.sh --source release` |
...
| Patch buckets | `vyos/` (3) → `ask/` (8) → `fixes/` (5) | `release/patches/` |
...
3. The `Pass: 16` count changes only when a new persistent patch is added or one is removed. Each change is its own commit.
