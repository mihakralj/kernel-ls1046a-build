...
static int Vlan_Get_Hash_Snapshot(int vlan_hash_index, int vlan_entries, PVlanCommand pVlanSnapshot)
{
	...
	slist_for_each(pVlanEntry, entry, &vlan_cache[vlan_hash_index], list)
	{
		...
		strscpy((char *)pVlanSnapshot->vlanifname, get_onif_name(pVlanEntry->itf.index), sizeof(pVlanSnapshot->vlanifname)); /* B6 P1.05 */
		strscpy((char *)pVlanSnapshot->phyifname, get_onif_name(pVlanEntry->itf.phys->index), sizeof(pVlanSnapshot->phyifname)); /* B6 P1.05 */
		...
		PStatVlanEntryResponse pStatVLANSnapshot, int *stat_tot_vlans)
{
	...
	*stat_tot_vlans = 0;
	slist_for_each(pStatVlanEntry, entry, &vlan_cache[stat_vlan_hash_index], list)
	{
		...
		strscpy((char *)pStatVLANSnapshot->vlanifname, get_onif_name(pStatVlanEntry->itf.index), sizeof(pStatVLANSnapshot->vlanifname)); /* B6 P1.05 */
		strscpy((char *)pStatVLANSnapshot->phyifname, get_onif_name(pStatVlanEntry->itf.phys->index), sizeof(pStatVLANSnapshot->phyifname)); /* B6 P1.05 */
