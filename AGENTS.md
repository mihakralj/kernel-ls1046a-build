...
Must report `Pass: 18   Fail: 0` and `0 SDK conflicts (266 files to install)`. A clean `patch-health` is **not sufficient** — `git apply` may report success even when a patch's hunk count is wrong and lines get silently truncated. After patch-health, also visually inspect the affected file:
...
| 100 | `fm-cc-copy-td-null-guard.patch` | Guard the unconditional `(struct en_exthash_info *)handle` cast at the head of `copy_td_to_ccbase()` against NULL / non-EHASH handles. When `/etc/cdx_pcd.xml` (no `external="yes"`) feeds `FM_PCD_CcRootBuild()` a CC-root tree entry with `h_CcNode == NULL` or a `t_FmPcdCcNode *` produced by the non-EHASH path, the first field load (`info->h_Ad` at offset 0x18) NULL-derefs and SIGSEGVs `dpa_app` at `copy_td_to_ccbase+0x68`. That fault cascades into `cdx_create_fragment_bufpool::failed to locate eth bman pool`, `BMan fragment buffer pool located by CDX [FAILED]`, and `cdx_module_init::start_dpa_app failed rc 11 / dpa_ipsec start failed`. Patch 098's EHASH redirect only helps when the userspace XML actually requests `external="yes"`; the on-target image does not, so this guard is the producer-side fix that keeps the tree build going for non-EHASH entries until userspace `fmc`/`fmlib`/`cdx_pcd.xml` are corrected in the consumer repo |
