...
Must report `Pass: 18   Fail: 0` and `0 SDK conflicts (262 files to install)`. A clean `patch-health` is **not sufficient** — `git apply` may report success even when a patch's hunk count is wrong and lines get silently truncated. After patch-health, also visually inspect the affected file:
...
| 102 | `arm64-ioremap-cache-ns-shim.patch` | (ask7) One-line `ioremap_cache_ns()` macro in `arch/arm64/include/asm/io.h` aliasing it to `ioremap_cache()`. Called by `bman_driver.c:194` and `qman_driver.c:451`. NXP's lf-6.x ships this helper using `PROT_NORMAL_NS` (NXP-private TLB attr); mainline lacks both the helper and the attr. The cacheable mapping is a strict superset for portal-CE use on this SoC running in EL2 (no secure-world traversal via these portals). Required for the lf-6.6.y SDK overlay to compile |

> **ask7 build fixes (2026-05-03):** the lf-6.6.y SDK overlay required two thin compat shims to compile against stock `linux-6.6.137` (lf-6.6.y itself ships ~6.6.52 + NXP-private API extensions). Patches `fixes/101` and `fixes/102` provide them; `bman_driver.o` (20096 B) and `sprint.o` (1960 B) build cleanly with both applied. The shims are minimal and isolated — no behaviour change for non-SDK paths.
